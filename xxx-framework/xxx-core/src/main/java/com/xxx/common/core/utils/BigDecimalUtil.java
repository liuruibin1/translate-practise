...
public class BigDecimalUtil {
     ...
     * @return
     */
    public static BigDecimal get10Power(int power) {
        BigInteger tenBI = new BigInteger("10");
        BigInteger decimalsPowBI = tenBI.pow(power);
        return new BigDecimal(decimalsPowBI.longValueExact());
    }
    ...
    public static BigDecimal add(BigDecimal... bigDecimals) {
        ...
        for (BigDecimal data : bigDecimals) {
            ...
            if (result == null) {
                ...
                result = BigDecimalUtil.add(result, data);
            }
        }
        return result;
    }
    ...
    public static BigDecimal add(Object number1, Object number2) {
        return BigDecimalUtil.add((BigDecimal) number1, (BigDecimal) number2);
    }
    ...
    public static BigDecimal sub(Object number1, Object number2) {
        return BigDecimalUtil.sub((BigDecimal) number1, (BigDecimal) number2);
    }
    ...
    public static BigDecimal multiply(BigDecimal number1, BigDecimal number2) {
        return BigDecimalUtil.multiply(number1, number2, true);
    }
    ...
    public static BigDecimal div(Object number1, Object number2, Integer power) {
        return BigDecimalUtil.div(number1, number2, power, true);
    }
    ...
    public static BigDecimal div(Object number1, Object number2, Integer power, boolean nullIsZero) {
        ...
        if (bigDecimal1.compareTo(BigDecimal.ZERO) == 0) return BigDecimal.ZERO;
        //被除数不能为0
        if (bigDecimal2.compareTo(BigDecimal.ZERO) == 0) return null;
        if (power == null) power = 8;
        return bigDecimal1.divide(bigDecimal2, power, RoundingMode.HALF_UP);
    }
    ...
    public static BigDecimal div(Object number1, Object number2) {
        return BigDecimalUtil.div(number1, number2, true);
    }
     ...
     * 取有效值，小数后一位
     * <pre>
     *     55432542.2431453312300000000 -> 55432542
     *     5543254235432545.5431453312300000000 -> 5543254235432546
     *     0.032345 -> 0.03
     *     0.0000017 -> 0.000002
     *     0.000084549 -> 0.00008
     *     -0.0000017 -> -0.000002
     ...
     * @return
     */
    public static BigDecimal toSignificant(BigDecimal num) {
        if (num.compareTo(BigDecimal.ONE) >= 0) {
            return num.setScale(0, RoundingMode.HALF_UP);
        } else {
            BigDecimal numABS = num.abs();
            String numStr = numABS.stripTrailingZeros().toPlainString();
            String decimal = numStr.substring(numStr.indexOf(".") + 1);
            int power = decimal.length();
            int i = 0;
            BigDecimal newNum;
            do {
                newNum = numABS.multiply(get10Power(i));
                i++;
            } while (newNum.compareTo(BigDecimal.ONE) < 0 && i <= power);
            return num.setScale(i - 1, RoundingMode.HALF_UP);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        BigDecimal b = new BigDecimal("222000.0000000000");
        System.out.println(b.stripTrailingZeros().toPlainString());*/

        BigDecimal num = BigDecimal.ZERO;

        num = new BigDecimal("55432542.2431453312300000000");
        System.out.println(toSignificant(num));

        num = new BigDecimal("5543254235432545.5431453312300000000");
        System.out.println(toSignificant(num));

        num = new BigDecimal("0.032345");
        System.out.println(toSignificant(num));

        num = new BigDecimal("0.0000017");
        System.out.println(toSignificant(num));

        num = new BigDecimal("0.000084549");
        System.out.println(toSignificant(num));
